...
![Burp Repeater request with storeId=1|whoami payload and command execution response](evidence/vuln-001-poc-request.png)
_Figure 1 — Burp Suite Repeater request showing the `storeId` parameter modified to `1|whoami`, with the HTTP 200 response returning the current OS user in the response body._
...
_Figure 1 — PoC request/response confirming OS command execution via the `storeId` parameter._
